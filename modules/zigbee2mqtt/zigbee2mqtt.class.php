...
//$st='OFF';
                ...
                $mqttsendvalue = '{"state": "' . $st . '",  "brightness": ' . $br . ',  "color": {"hex": "#' . $value . '"}}';
            }
...
//if (($rec['PAYLOAD_OFF'])&& ($value=="0"))  $json=array( $rec['METRIKA']=> $rec['PAYLOAD_OFF']);
            ...
            if ($value == "1") {
                $json = array($rec['COMMAND_VALUE'] => $rec['PAYLOAD_ON']);
                SQLExec('update zigbee2mqtt_devices set state="1" where ID="' . $rec['DEV_ID'] . '"');
            }
            if ($value == "0") {
                $json = array($rec['COMMAND_VALUE'] => $rec['PAYLOAD_OFF']);
                SQLExec('update zigbee2mqtt_devices set state="0" where ID="' . $rec['DEV_ID'] . '"');
            }
            $jsonvalue = json_encode($json);
...
//if (($rec['PAYLOAD_OFF'])&& ($value=="0"))  $json=array( $rec['METRIKA']=> $rec['PAYLOAD_OFF']);
                        ...
                        if ($value == "1") {
                            $json = array($rec[$i]['COMMAND_VALUE'] => $rec[$i]['PAYLOAD_ON']);
                            SQLExec('update zigbee2mqtt_devices set state="1" where ID="' . $rec['DEV_ID'] . '"');
                        }
                        if ($value == "0") {
                            $json = array($rec[$i]['COMMAND_VALUE'] => $rec[$i]['PAYLOAD_OFF']);
...
//$zz=substr(explode('/',$this->config['MQTT_QUERY'])[0],1);
        ...
        if (strpos($path, '/bridge/state') > 0) {
            $msgtype = 'bridge_state';
        } else if (strpos($path, '/bridge/config') > 0) {
            $msgtype = 'bridge_config';
        } else if (strpos($path, '/bridge/networkmap/raw') > 0) {
            $msgtype = 'raw_map';
        } else if (strpos($path, '/bridge/networkmap') > 0) {
            $msgtype = 'graphwiz';
        } else if (strpos($path, '/bridge/networkmap/raw/nodes') > 0) {
            $msgtype = 'rawnodes';
        } else if (strpos($path, '/bridge/log') > 0) {
            $msgtype = 'log';
        } else {
...
//{"type":"groups","message":{"1":{"friendly_name":"232323"},"2":{"friendly_name":"group1"},"3":{"friendly_name":"group1"},"4":{"friendly_name":"group1"}}}
                ...
                $arr['TITLE'] = $path;
                if ($msgtype == 'device_state' || $msgtype == 'raw' || $msgtype == 'graphviz') {
                    $arr['MESSAGE'] = $path . ":" . $value;
                } else {
                    $arr['MESSAGE'] = $value;
                }
                $arr['TYPE'] = $msgtype;
...
//debmes(strpos(substr($path),"/set/"), 'z2m_settt')
        ...
        if ((substr($path, -4) == "/set") || (strpos($path, "/set/") > 0)) {
            debmes('путь содержит set, его мы записывать не будем, чтобы не было колизии', 'z2m_settt');
        } else {
////основной запрос
...
//проверяем, новая тема  по группе или устройству
                ...
                if ($grl['ID']) {
                    $rec['SELECTTYPE'] = 'group';
                    $rec['SELECTVENDOR'] = 'group';
                }
...
*/
            ...
            if ($rec['LINKED_OBJECT'] && $rec['LINKED_PROPERTY']) {
                ...
                if (($rec['PAYLOAD_ON']) || $rec['PAYLOAD_OFF']) {
                    if ($value == $rec['PAYLOAD_ON']) {
                        $newvalue = 1;
                        SQLExec('update zigbee2mqtt_devices set state="1" where ID="' . $rec['DEV_ID'] . '"');
                    }
                    if ($value == $rec['PAYLOAD_OFF']) {
                        $newvalue = 0;
                        SQLExec('update zigbee2mqtt_devices set state="0" where ID="' . $rec['DEV_ID'] . '"');
                    }
...
//if (ZMQTT_DEBUG=="1" ) debmes('Заменили  '.$value. "  на ". $newvalue, 'zigbee2mqtt');
                ...
                (($rec['PAYLOAD_TRUE']) || $rec['PAYLOAD_FALSE']) {
                    if ($value == $rec['PAYLOAD_TRUE']) {
                        $newvalue = 1;
                        SQLExec('update zigbee2mqtt_devices set state="1" where ID="' . $rec['DEV_ID'] . '"');
                    }
                    if ($value == $rec['PAYLOAD_FALSE'] || $value == "") {
                        $newvalue = 0;
                        SQLExec('update zigbee2mqtt_devices set state="0" where ID="' . $rec['DEV_ID'] . '"');
                    }
                } else {
...
//echo '123';
            ...
            $out['DEVICE_TITLE'] = $res['TITLE'];
            $out['DEVICE_ID'] = $res['ID'];
...
// echo "</script>";
        ...
        if ($this->view_mode == 'cycle_start') {
            ...
            if ((time() - gg('cycle_zigbee2mqttRun')) > 360 * 30) {
...
//if ($new_value=='true')  {settype($new_value, 'bool');}
            if ($new_value == 'false') {
                $new_value = "";
                settype($new_value, 'bool');
            }


            if ($rec['METRIKA'] == 'current_heating_setpoint') {
...
//   if (ZMQTT_DEBUG=="1" ) debmes('total:'.$total, 'zigbee2mqtt');
        for ($i = 0; $i < $total; $i++) {
            ...
            $cdev = $json[$i]->{'ieeeAddr'};
            $friendly_name = $json[$i]->{'friendly_name'};
...
//$res2['MODEL']=$json[$i]->{'type'};
                ...
                if ($friendly_name != '' && $friendly_name != $res2['TITLE']) {
                    $res2['TITLE'] = $friendly_name;
                }
